    ...
    class task{
        ...
        friend bool loadTask(task mytask[], char *userfile, char *inputPassword);
...
bool loadTask(task *mytask,char *userfile,char *inputPassword);//改成了bool型，成功登录返回1，失败返回0
void saveTask(char *password,task *mytask,char *userfile);//改成了bool型，成功保存返回1，失败返回0
#endif
