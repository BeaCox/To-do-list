...
int main(int argc, char * argv[]){
    ...
    while ((opt = getopt(argc, argv, optstring)) != -1) {
            ...
            case 'r'://循环运行程序
	    {
		    ...
		    while(!loadTask(myTask,name2File(inputName),inputPassword))
		    {
			    login(inputName,inputPassword);
	            }
                }
                ...
                cout << "请输入命令(输入help获取帮助信息)：" <<endl;
                getCommand(commandcount, commands);//获取命令
                //将命令转换成数字
                if (strcmp(commands[0],help) == 0) opt_r = 1;
                if (strcmp(commands[0], add) == 0) opt_r = 2;
                if (strcmp(commands[0], show) == 0) opt_r = 3;
                if (strcmp(commands[0], del) == 0)opt_r = 4;
                if (strcmp(commands[0], quit) == 0) opt_r = 5;
                		...
                		switch (opt_r) {
				       	...
				       	myTask[task::taskLive].setTask();
                            ...
                            case 5: {
                                ...
                                saveTask(inputPassword,myTask,name2File(inputName));
                                pthread_cancel(id);
				break;
                            }
                            default: {
